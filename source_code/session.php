...
if (isset($session_id)) {
    ...
    $sessionqry = mysql_query("SELECT kff1006, kff1005, kd
                          FROM $db_table
                          WHERE session=$session_id
                          ORDER BY time DESC", $con) or die(mysql_error());
    $result = mysql_query("SELECT * 
                           FROM `overall` 
                           WHERE `session` = $session_id AND (`id` = '789ecbb09cf9ca79726f522d9782623f' or `id` = 'bfb76f71614533352b6107609ab99ff0')", $con) or die(mysql_error());
    ...
    $session_id = "";
    $imapdata = "new google.maps.LatLng(44.2336, -76.5043)";
    $setZoomManually = 1;
}
...
<html lang="en">
    <head>
        ...
        <title>Travel Viewer</title>
        <meta name="description" content="Travel Viewer">
        <meta name="author" content="Matt Nicklay, Annie Lee, Ray Wen, Jenne Zhang">
        <!--<link rel="stylesheet" href="//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css">-->
        <link href="./static/css/icon.ico" type="img/ico" rel="icon">
        ...
        window.onload = function () {
            var map = new google.maps.Map(document.getElementById('map-canvas'), {
              zoom: 5,
              center: new google.maps.LatLng(44.2336, -76.5043),
            ...
            google.maps.event.addListener(flightPath, 'mouseover', function (e) {
                ...
                infoWindow.setContent('<h5>Driver Current Data</h5>'+'Latitude: ' + latitude + '<br />Longitude: ' + longitude);
        ...
        $(document).ready(function(){
            ...
            var flotData = [
                            ...
                            { data: s2, label: <?php echo $v2_label; ?>, yaxis: 1 } //change yaxis: from 2 to 1
                           ];
        ...
        <div class="navbar navbar-default navbar-fixed-top navbar-inverse" role="navigation">
            <div class="container">
                <div class="navbar-header">
                    <img src="static/css/logo.png" height="40px" >
                    <a class="navbar-brand" href="session.php">Travel Viewer</a>
                </div>
                </div>
            </div>
...
<?php } /* END: if(isset($session_id) && !empty($session_id)) */?>
                    ...
                    <?php if ($setZoomManually === 0) { ?>
                    <h4>Select a Variable to Plot</h4>
                    <div class="row center-block" style="padding-top:3px;">
                        <form method="post" role="form" action="url.php?makechart=y&seshid=<?php echo $session_id; ?>" id="formplotdata">
                            <select required data-placeholder="Choose OBD2 data..." class="form-control chosen-select" size="<?php echo $numcols; ?>" style="width:100%;" id="plot_data" onsubmit="onSubmitIt" name="plotdata[]">
                                <option value=""></option>
                    ...
                    <h4>Driver Profile Score</h4>
                    <div class="row center-block">
                        <?php if($setZoomManually === 0){?>
                            <div class="table-responsive">
                                <table class="table">
                                    <h5>
                                    If the driver score is high, then the driver is not a calm driver in this journey.
                                    </h5>
                                    <thead>
                                        <tr>
                                    ...
                                    while($row = mysql_fetch_array( $result )) {
                                        extract($row);
                                        $total_score = round($total_score, 2)*100;
                                        $speed = round($speed, 2);
                                        $accel = round($accel, 2);
                                        $break = round($accel, 2);
                                        $angle = round($accel, 2);
                                        ...
                                        else if($total_score<=60 AND $total_score>30){
                                            $scoreT = "MODERATE";
                                        }
                                        else{
                                            $scoreT = "AGGRESSIVE";
                                        ...
                                        else if($speed<=0.60 AND $speed>0.30){
                                            $scoreS = "MODERATE";
                                        }
                                        else{
                                            $scoreS = "AGGRESSIVE";
                                        ...
                                        else if($accel<=0.60 AND $accel>0.30){
                                            $scoreAc = "MODERATE";
                                        }
                                        else{
                                            $scoreAc = "AGGRESSIVE";
                                        ...
                                        else if($break<=0.60 AND $break>0.30){
                                            $scoreB = "MODERATE";
                                        }
                                        else{
                                            $scoreB = "AGGRESSIVE";
                                        ...
                                        else if($angle<=0.60 AND $angle>0.30){
                                            $scoreAng = "MODERATE";
                                        }
                                        else{
                                            $scoreAng = "AGGRESSIVE";
                                        }
                                        echo "<tr><td><center>$total_score %</center></td> 
                                                  <td>$scoreS</td>
                                                  <td>$scoreAc</td> 
                                                  <td>$scoreB</td>
                                                  <td>$scoreAng</td>"; 
