...
$result = mysqli_query($con,"SELECT count(roadCondition.id) as slipper FROM roadCondition WHERE cast(kff1005 as decimal)<=cast($high1005 as decimal) AND cast(kff1005 as decimal)>= cast($low1005 as decimal)  AND cast(kff1006 as decimal)<= cast($high1006 as decimal) AND cast(kff1006 as decimal)>=cast($low1006 as decimal) AND type=2");
