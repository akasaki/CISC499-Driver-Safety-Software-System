...
//get surrounding id
$result=mysqli_query($con,"SELECT id, count(id) as aggDriver from raw_logs where id!='bfb76f71614533352b6107609ab99ff0' AND (kff1005 between $kff1005-0.004 and $kff1005+0.0001) AND (kff1006 between $kff1006-0.0002 and $kff1006+0.0002) AND id IN (select `id` from overall group by `id` having avg(`total_score`)>=0.5)");
